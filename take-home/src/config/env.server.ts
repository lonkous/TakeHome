  ...
  if (!parsed.success) {
    ...
    if (typeof window !== 'undefined' || process.env.EXPO_OS === 'web') {
      _env = {
        ...
        AZURE_COSMOS_DB_DATABASE_NAME: process.env.AZURE_COSMOS_DB_DATABASE_NAME ?? '',
...
export const env = new Proxy({} as z.infer<typeof EnvSchema>, {
  get(_t, p) { return (getEnv() as any)[p]; },
});

const config = new Proxy(
  {} as { cosmos: { endpoint: string; key: string; database: string; containers: { datas: string } } },
  {
    get(_t, p) {
      const e = getEnv();
      if (p === 'cosmos') {
